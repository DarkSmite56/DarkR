let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  Contoh penggunaan: ${usedPrefix}math medium
    ...
    conn.math[id] = [
        ...
        math, 4,
        setTimeout(() => {
            if (conn.math[id]) conn.sendButton(m.chat, `Waktu habis goublog!\nBofoh kali Jawabannya itu ${math.result}`, author, null, [['again', `${usedPrefix}${command} ${math.mode}`], ...buttons], conn.math[id][0])
            delete conn.math[id]
        }, math.time)
    ]
}
