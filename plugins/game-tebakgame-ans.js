...
export async function before(m) {
    ...
    if (!(id in this.tebakgame))
        return conn.sendButton(m.chat, 'Soal itu udh gk ada', author, null, buttontebakgame, m)
    if (m.quoted.id == this.tebakgame[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Betul anjay!*\n+${this.tebakgame[id][2]} XP`, author, null, buttontebakgame, m)
            clearTimeout(this.tebakgame[id][3])
            delete this.tebakgame[id]
        } else if (similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold)
            m.reply(`*Dikit Lagi itu bg!*`)
        else
            conn.sendButton(m.chat, `*Salah goublog!*`, author, null, [
                ['Hint', '/hgame'],
                ['Nyerah', 'menyerah']
            ], m)
    }
